...
def test_capture_script_runs_on_seed() -> None:
    import os
    ...
    result = subprocess.run(
        [sys.executable, str(REPO_ROOT / "scripts/capture_overrides.py"),
         ...
         "--adversarial", "data/adversarial/adversarial_hitl_check.jsonl"],
    ...
    check = REPO_ROOT / "data/adversarial/adversarial_hitl_check.jsonl"
    assert sum(1 for line in check.read_text().splitlines() if line.strip()) == 2
    check.unlink()
